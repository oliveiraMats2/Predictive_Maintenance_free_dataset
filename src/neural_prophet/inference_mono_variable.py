...
if __name__ == '__main__':
    ...
    df_train, _ = adjust_dataframe_for_train.dataset_split(df_, split=configs["train_test_split"])
    ...
    # start_date = df_train["ds"].tolist()[-1]
    # configs["window_prevision"]["start_date"] = start_date

    train_model.load(f'weighted_history/{configs["name"]}_{configs["select_feature"]}.np')

    m = train_model.neural_prophet

    df_eixo_time = adjust_dataframe_for_train.create_dataframe(**configs["window_prevision"])
